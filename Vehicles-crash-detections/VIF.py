...
import cv2
import math
from denseOpticalFlow import OpticalFlow_process
...
class ViF:
    ...
    def create_block_hist(self, flow, N, M):
        ...
        for y in np.arange(6, height - B_height - 5, B_height):
            for x in np.arange(6, width - B_width - 5, B_width):
                block_hist = self.create_hist(
                    flow[y : y + B_height - 1, x : x + B_width - 1]
                )
    ...
    def process(self, frames):
        ...
        for i in range(
            0, len(frames) - self.video_subsampling - 5, self.video_subsampling * 2
        ):
            ...
            # process optic flow
            _, m1, _ = OpticalFlow_process(prev_f, curr_f)
            _, m2, _ = OpticalFlow_process(curr_f, next_f)
